...
export default function TempleCard({ temple, onDelete }) {
    ...
    // Handle temple.user as populated object { _id: "..." } or plain string ID
    const templeOwnerId = temple.user?._id
        ? temple.user._id.toString()
        : temple.user?.toString();
    const isAdmin = user && templeOwnerId && user._id?.toString() === templeOwnerId;
